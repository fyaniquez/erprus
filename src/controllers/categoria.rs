...
/// autor: fyaniquez
/// fecha: 2021-07-11 21:43:03.192553648 -04:00
///
use crate::handlers::categoria::*;
use crate::models::categoria::Categoria;
use crate::views::categoria::*;
use crate::State;
use sqlx::PgPool;
use tide::Request;
...
pub async fn ctrl_create(mut req: Request<State>) -> tide::Result {
    let db_pool: PgPool = req.state().db_pool.clone();
    let parametros: Categoria = req.body_form().await?;
...
pub async fn ctrl_update(mut req: Request<State>) -> tide::Result {
    ...
    let mut actualizado: Categoria = req.body_form().await?;
    let row = hndl_get(&db_pool, id).await?;
    let res = match row {
        ...
        Some(row) => {
            ...
            actualizado.capitulo_id = row.capitulo_id;
