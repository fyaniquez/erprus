/// controller unidad
/// autor: fyaniquez
/// fecha: 2021-07-18 21:31:53.592664605 -04:00
///
use crate::handlers::unidad::*;
use crate::models::unidad::Unidad;
use crate::views::unidad::*;
use crate::State;
use sqlx::PgPool;
use tide::Request;
...
pub async fn ctrl_create(mut req: Request<State>) -> tide::Result {
    let db_pool: PgPool = req.state().db_pool.clone();
    let parametros: Unidad = req.body_form().await?;
...
pub async fn ctrl_update(mut req: Request<State>) -> tide::Result {
    ...
    let mut actualizado: Unidad = req.body_form().await?;
    let row = hndl_get(&db_pool, id).await?;
    let res = match row {
        None => return ctrl_list(req).await,
        Some(row) => {
            actualizado.id = row.id;
            actualizado.sigla = row.sigla;
