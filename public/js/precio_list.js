...
const borrar_fila = async (fila) => {
    ...
    if (conf == null) return;
    const response = await fetch(`/erprus/precios/${fila}`, {
        method: 'DELETE'
    });
...
const ir_a = (pagina, filtro, longitud) => {
    let url = `/erprus/precios?pagina=${pagina}&filtro=${filtro}&longitud=${longitud}`;
...
const onClick_pagebar = (e) => {
    if (e.target.className && e.target.className == "page") {
        ...
        window.location.replace(encodeURI(`/erprus/precios?pagina=${pagina}&filtro=${filtro}&longitud=${longitud}`));
    }
}
const onChange_longitud = (e) => {
    ...
    window.location.href = encodeURI(`/erprus/precios?pagina=${pagina}&filtro=${filtro}&longitud=${longitud}`);
}
